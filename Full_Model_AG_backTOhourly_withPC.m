...
% kLayerThick_s = 3; % Thickness of surface layers (m)
...
%         drawnow
    ...
    % Pressures
    mSVP_s = 6.11 * exp((2.5e6 / 461) .* (1 / 273.15 - 1 ./ (273.15 + mT_s_Act_s))); % Saturation vapor pressure of the surface
...
%     mL_in = kSigma .* ((273.15 + mT_a) .^ 4) .* (0.585 + 0.062 * mVP_a) .* mGlacMask; %Molg & Hardy - another option
    ...
    mQ_S = (kRho_a_sl .* kC_a .* mK_H .* (mP_a ./ kP_a_sl) .* mU .* (mT_a - mT_s_Act_s)) ...
        .* mGlacMask; %P_a
    ...
    % Q_G (W m^-2)
    mQ_G = ( kK * mK_s .* (mT_s_Act_2 - mT_s_Act_s) ./ kLayerThick_s ) ./ kLayerThick_s .* kTimeStep_sec .* mGlacMask;     % From Paterson, 1994; The Physics of Glaciers, 3rd ed., pg. 206 (Eq. 5)
%     NEW: mQ_G = (kK * (mT_s_Act_3 - mT_s_Act_s) ./ kLayerThick_s ) .* mGlacMask;  
