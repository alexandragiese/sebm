function [R1, R2, RAve, R_k ] = MC_Model (GUI_Input,vP_d_th,y,g,mc,ct,fl )
tic
% close all
...
if strcmp(sGCM,'HAR')
    ...
    [ vSW_in, vU, vT_a, ~, vP_a, vRH, ~, vLW_in, stTime, Data  ] = ExtractHAR1( kLat, kLong, sTempRes, sDataDirectory ); %pulls in all HAR output
...
% Roughness length ADD VARIATION HERE
...
kAlpha_fs   = GUI_Input.Alpha_fs;    % ADD VARIATION HERE                                                        % 0.9 from Molg and Hardy, 2004; 0.85 from Physics of Glaciers
...
kAlpha_i    = GUI_Input.Alpha_i;     % ADD VARIATION HERE                                                        % 0.45 from Molg and Hardy, 2004; 0.35 from Physics of Glaciers; 0.30 in Johnson & Rupper
...
kRainThreshold = GUI_Input.RainThreshold;  % ADD VARIATION HERE
...
%ERIC'S Stability correction (only applied for sub-daily time steps) replaced
...
%     across glacier surface saved for all timesteps (if I want access to
%     prior 24 I need to keep a variable of them)
%       if any(any(isnan(mT_s)))==1  % if NaN this is a
% %       problem but not if the glacier is so high and cold that the glacier
% %       surface temp on day 1 (first 24 time steps) is < -15 C
%         fprintf('nan temp for g = %d\n',g)
%         keyboard
%       end    
...
% Save constants varied in Monte Carlo experiments
R_k.Alpha_fs = kAlpha_fs;                                                             % 0.9 from Molg and Hardy, 2004; 0.85 from Physics of Glaciers
R_k.kAlpha_i = kAlpha_i; 
R_k.kRainThreshold = kRainThreshold;
R_k.Z_0m_snow  = kZ_0m_snow;
R_k.Z_0m_ice   = kZ_0m_ice;
R_k.Z_0T_snow  = kZ_0T_snow;
R_k.Z_0T_ice   = kZ_0T_ice;
R_k.Z_0q_snow  = kZ_0q_snow;
R_k.Z_0q_ice   = kZ_0q_ice;
R_k.LRA = GUI_Input.lra; 
R_k.PC  = GUI_Input.PC;
...
%     save([GUI_Input.output_filename, num2str(iGlacierNumber),'_',basinname,'X.mat'])
    save([GUI_Input.output_filename, num2str(iGlacierNumber),'_MC',num2str(mc),'_CALiter',num2str(ct),'_',fl,'.mat']) %MC iteration, calibration iteration, flag for final or still calibrating
end
end
