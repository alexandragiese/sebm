function [R1, R2, RAve ] = Full_Model_AG_spinup( GUI_Input,y,g )
...
kAlpha_fi   = 0.53; %AG. was 0.55;        was 0.40                                     % 0.53 from Molg and Hardy, 2004; 0.55 from Physics of Glaciers; 0.40 in Johnson & Rupper
% Albedo of glacier ice (unitless)
kAlpha_i    = 0.34; %AG. was 0.35;        was 0.30                                      % 0.45 from Molg and Hardy, 2004; 0.35 from Physics of Glaciers; 0.30 in Johnson & Rupper
...
%     mSnowClock(mT_a <= kRainThreshold & mPrecip > kPrecipThresh) = 0;
    mSnowClock(mT_a <= kRainThreshold) = 0;
    ...
    % Aging snow
    mAlpha_s    = kAlpha_fi + (kAlpha_fs - kAlpha_fi) .* exp(-ceil(mSnowClock) / kE_t);      % From Molg and Hardy, 2004 (Eq. 3) note: whole # days in numerator
    % Snow depth albedo
...
mStabCorr = ones(size(mGlacMask)); 
       ...
       mStabCorr(mR_b < 0) = 1; %for low wind speeds, stability correction not applied
       mStabCorr(mU < 1 ) = 1; %for low wind speeds, stability correction not applied     
    ...
    if mSnowThickness_2 > kLayerThick_2
        disp('More than 3m snow! Aborting.')
        returm
    end
...
%% Surface temperature: equations 11 and 12 of Arnold et al., 2006: roundabout approach
    ...
    % ERIC'S had Theoretical surface temperature (K) and Actual surface temperature (can't be above zero) (K)
%     mT_s_Act_2(mT_s_Act_2 > 0) = 0; %Energy not conserved!!!
...
% ERIC'S: mT_s(mT_s<-50) = -50; %Energy not conserved!!!
...
% % Surface temperature: these are equivalent within machine precision
    mT_s(mT_s > 0) = 0; 
%     mT_s = mT_s - mQ_m * kTimeStep_sec ./ (kC_i .* mRho_s .* kLayerThick_s) .* mGlacMask;
%     mT_s (mT_s > -(1*10^(-12)) & mT_s < (1*10^(-12)) ) = 0; %reset to exactly zero
...
% end    
    ...
    mSnowDepth(mSnowDepth < 0) = 0; %IS: mSnowDepth(mSnowDepth < 0) + mXS_SnowMelt(mSnowDepth < 0)
    % Ice melt (m)
...
if t == 17544
    TH_i.SnowThickness_s = nanmean(mSnowThickness_s(mGlacMask==1));
    TH_i.SnowThickness_2 = nanmean(mSnowThickness_2(mGlacMask==1));
    TH_i.SnowDepth = nanmean(mSnowDepth(mGlacMask==1));
    TH_i.SnowMelt  = nanmean(mSnowMelt(mGlacMask==1));
    TH_i.IceSurface = nanmean(mIceSurface(mGlacMask==1));
    TH_i.IceMelt = nanmean(mIceMelt(mGlacMask==1));
...
if t == kEnd
    TH_f.SnowThickness_s = nanmean(mSnowThickness_s(mGlacMask==1));
    TH_f.SnowThickness_2 = nanmean(mSnowThickness_2(mGlacMask==1));
    TH_f.SnowDepth = nanmean(mSnowDepth(mGlacMask==1));
    TH_f.SnowMelt  = nanmean(mSnowMelt(mGlacMask==1));
    TH_f.IceSurface = nanmean(mIceSurface(mGlacMask==1));
    TH_f.IceMelt = nanmean(mIceMelt(mGlacMask==1));
...
%     m3T_a m3Precip m3Q_net m3Q_m m3T_a m3Precipitation ...
%     m3TotalMelt m3TotalAccum mGlacMask mGlacAlt stTime ...
%     mTotalMelt_1975 mTotalAccum_1975 mTotalSub mTotalEvap Data mDebMask kAWS_Alt m3T_s m3T_b ...
%     modelMB geodeticMB geoMB_sigma  
...
clearvars -except R2 RAve R_Geo GUI_Input iGlacierNumber mTotalMelt mTotalAccum mGlacMask mGlacAlt stTime ...
    mTotalSub mTotalEvap Data kAWS_Alt modelMB geodeticMB geoMB_sigma TH_i TH_f
%NEED TO ADD SOME THICKNESS TERMS!

if GUI_Input.threeDsave == 1 % IF BIG
    save([GUI_Input.output_filename, num2str(iGlacierNumber),'_3D_90m_exploreSW.mat'],'-v7.3')
else
    save([GUI_Input.output_filename, num2str(iGlacierNumber),'_SUand3Dtest2.mat'])
end
