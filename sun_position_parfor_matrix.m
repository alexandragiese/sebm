function [zenith, azimuth] = sun_position_parfor_matrix(time, latitude, longitude, altitude)
...
%% Begin output modifiation for parfor loop

zenith = sun.zenith;
azimuth = sun.azimuth;

%% End output modification for parfor loop
...
apparent_elevation = zeros(size(true_elevation));
apparent_elevation(true_elevation > -5) = true_elevation(true_elevation > -5) + refraction_corr(true_elevation > -5);
apparent_elevation(true_elevation <= -5) = true_elevation(true_elevation <= -5);
...
%     var = var + max_interval;
% end
var(var<min_interval) = var(var<min_interval) + max_interval(var<min_interval);
