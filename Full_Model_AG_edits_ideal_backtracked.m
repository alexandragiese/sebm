function [R1, R2, RAve ] = Full_Model_AG_edits( GUI_Input,g )
...
kAlpha_fi   = 0.40; %AG. was 0.55;                                              % 0.53 from Molg and Hardy, 2004; 0.55 from Physics of Glaciers; 0.40 in Johnson & Rupper
% Albedo of glacier ice (unitless)
kAlpha_i    = 0.30; %AG. was 0.35;                                              % 0.45 from Molg and Hardy, 2004; 0.35 from Physics of Glaciers; 0.30 in Johnson & Rupper
...
% Emissivity
kE_i        = 1; %AG. was 0.98;                                                 % 1 in Johnson & Rupper                                                     
...
mT_s = zeros(size(mGlacMask)).*mGlacMask;
kLayerThick_s = 3; % Thickness of surface layer (m)
...
%         returm
%     end
% KEEP TRACK OF TOTAL AMOUNT OF SNOW SOMEWHERE? mSnowDepth
...
%     mL_in = kSigma .* ((273.15 + mT_a) .^ 4) .* (0.585 + 0.062 * mVP_a) .* mGlacMask; %Molg & Hardy - another option
    ...
    % Q_G (W m^-2)
%     OLD: mQ_G = ( kK * mK_s .* (mT_s_Act_2 - mT_s_Act_s) ./ kLayerThick_s ) ./ kLayerThick_s .* kTimeStep_sec .* mGlacMask;     % From Paterson, 1994; The Physics of Glaciers, 3rd ed., pg. 206 (Eq. 5)
    mQ_G = ( kK * mK_s .* (mT_b - mT_s) ./ kLayerThick_s ) ./ kLayerThick_s .* kTimeStep_sec .* mGlacMask;     % From Paterson, 1994; The Physics of Glaciers, 3rd ed., pg. 206 (Eq. 5)
    %     NEW: mQ_G = (kK * (mT_b - mT_s) ./ kLayerThick_s ) .* mGlacMask;  
    % Remove NaNs and stuff
    mQ_L(isnan(mQ_L) == 1) = 0;
    % Net Energy (W m^-2) 
    mQ_net = (mS_net + mL_out + mL_in + mQ_S + mQ_L + mQ_P + mQ_G) .* mGlacMask;

%% Surface temperature: compute melt & Ts from energy balance?
    %% Surface temperature
    
% % Calculate change in temperature of top 2 layers for this timestep (K)   
% mDelta_T_s = (( mK_s .* (mT_s_Act_2 - mT_s_Act_s) ./ kLayerThick_s ) ./ kLayerThick_s + (mQ_net - mQ_G) ./ (kC_i .* mRho_s .* kLayerThick_s)) .* kTimeStep_sec .* mGlacMask;
...
mT_s = mT_s +(( mK_s .* (mT_b - mT_s) ./ kLayerThick_s ) ./ kLayerThick_s + (mQ_net - mQ_G) ./ (kC_i .* ...
...
%% Melt energy (W m^-2)
    ...
    mT_s(mT_s > 0) = 0; %<-- replace with below... when model works
...
%% Freezing of rain  
    ...
    RAve.hr_count = hr_count; %1x13 vector
    ...
    RAve.SnowMelt = nanmean(mSnowMelt(mGlacMask==1)); %m snow
    RAve.IceMelt  = nanmean(mIceMelt(mGlacMask==1));  %m ice
