function [R1, R2, RAve] = Full_Model_Nov15_2019email( GUI_Input )
...
% mDebrisMask = load('ALOS_90m_Debris_Mask_Ind_Gang_Brahm1.mat','mDebrisMask_90m'); % Added 11/15/19 -ESJ
...
%         kLayerThick_s ) ./ kLayerThick_s .* kTimeStep_sec .* mGlacMask;     % From Paterson, 1994; The Physics of Glaciers, 3rd ed., pg. 206 (Eq. 5)
    mQ_G = (kK * (mT_s_Act_2 - mT_s_Act_s) ./ kLayerThick_s ) .* mGlacMask;  
...
%     mRunoff = mEn_Diff * (kTS_mins * 60) / (kL_f * 1000);
    ...
    RAve.T_s_3(t) = nanmean(mT_s_Act_3(mGlacMask==1));    
...
clearvars -except R1 R2 RAve R_Geo GUI_Input iGlacierNumber sMicroPhysics ...
    ...
    mTotalMelt_1975 mTotalAccum_1975 mTotalSub mTotalEvap m3TotalMelt ...
    m3TotalAccum Data
...
% %             save([GUI_Input.output_filename,'origEric_gl',num2str(GUI_Input.glacier_number),'_inside.mat'])
% % 
